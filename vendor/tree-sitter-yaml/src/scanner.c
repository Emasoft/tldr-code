...
// ---------------------------------------------------------------------------
// VENDORED PATCH (tldr-code) — int32 row/col counters, 31 lines, upstream
// tree-sitter-yaml 0.7.0 otherwise verbatim.
//
// UPSTREAM DEFECT (tree-sitter-grammars/tree-sitter-yaml issue #49, open):
// this scanner tracked the current source row in int16_t fields. cur_row is
// incremented on every newline (adv_nwl / skp_nwl) and every block-structure
// decision hangs on `bool has_nwl = scanner->cur_row > scanner->row`; once a
// source reaches row 32768 (0-indexed) = 2^15 the increment overflows
// negative, has_nwl flips false, and the parser's error recovery swallows the
// entire remaining input into one root ERROR node — any .yaml past 32768
// lines parsed to garbage. The serialized scanner state also persisted the
// row as int16_t, so the wrapped value stuck across tokens.
//
// THE PATCH (behavior-identical below row 32768 — byte-identical trees):
//   1. the 9 scalar row/col fields of `Scanner` (row, col, blk_imp_row,
//      blk_imp_col, blk_imp_tab and the temp end_row/end_col/cur_row/cur_col)
//      widened int16_t -> int32_t;
//   2. serialize()/deserialize() widened to match (the 5 persisted scalar
//      fields move from 10 to 20 bytes; the 1024-byte serialization buffer
//      leaves slack for ~251 stack entries instead of ~253 — ample);
//   3. the bgn_row/bgn_col locals in the scan function widened to follow the
//      struct fields they are read from.
// The ind_typ_stk/ind_len_stk stacks stay int16_t (token kinds and indent
// columns — the same upstream shapes).
//
// Re-vendor: copy the new upstream crate's src/ over this directory and
// re-apply the steps above (see this crate's README.md). Upstream issue:
// https://github.com/tree-sitter-grammars/tree-sitter-yaml/issues/49
// ---------------------------------------------------------------------------
...
typedef struct {
    // VENDORED PATCH: int16_t -> int32_t (row 32768 overflow, see the file
    // header; issue #49).
    int32_t row;
    int32_t col;
    int32_t blk_imp_row;
...
static unsigned serialize(Scanner *scanner, char *buffer) {
    size_t size = 0;
    // VENDORED PATCH: the 5 persisted scalars serialize as int32_t to match
    // the widened Scanner fields (see the file header; issue #49).
...
static void deserialize(Scanner *scanner, const char *buffer, unsigned length) {
    ...
    if (length > 0) {
        size_t size = 0;
        // VENDORED PATCH: the 5 persisted scalars deserialize as int32_t to
        // match serialize() (see the file header; issue #49).
...
static bool scan(Scanner *scanner, TSLexer *lexer, const bool *valid_symbols) {
    ...
    // VENDORED PATCH: int16_t -> int32_t, follows the widened Scanner fields
    // they are read from (see the file header; issue #49).

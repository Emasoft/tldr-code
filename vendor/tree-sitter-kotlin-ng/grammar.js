...
module.exports = grammar({
  ...
  conflicts: $ => [
    ...
    // tldr vendored patch (B-kotlin-grammar): `suspend` is now also a
    // _reserved_identifier (arg-name), so the existing function_modifier vs
    // type_modifiers ambiguity must also admit the identifier reading.
    ...
    // tldr vendored patch (B-kotlin-grammar): the remaining modifier-keyword
    // groups whose spellings were added to _reserved_identifier.
  ...
  rules: {
    ...
    _reserved_identifier: $ => prec.dynamic(1, alias(
      choice(
        ...
        'operator',
        'value',
        // tldr vendored patch (B-kotlin-grammar): soft/modifier keywords that
        // are legal identifiers outside modifier position (Kotlin spec
        // `simpleIdentifier`). `suspend` is the one that hits real code
        // (coroutines named-arg lambdas, e.g. `g(suspend = { ... })`); peers
        // added for parity. Disambiguated from modifier position by the
        // existing prec.dynamic(1) + dedicated *_modifier rules.
        'suspend',
        'tailrec',
        'infix',
        'inline',

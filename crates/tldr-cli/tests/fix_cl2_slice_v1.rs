...
fn rust_detect_cycles_backward_not_full_block() {
    // detect_cycles 860..~905; criterion 870 (`if visited.contains(start_node)`).
    // Line numbers re-synced to the current deps.rs fixture — the
    // `detect_cycles` function drifted; the hardcoded 862 then pointed at a
    // comment line rather than the criterion. The criterion / def lines are
    // otherwise unchanged in shape.
    assert_backward_precise(RUST_DEPS, "detect_cycles", 870, 887, "rust/IT3-rust-04");
}

#[test]
fn rust_detect_cycles_backward_excludes_unrelated_def() {
    // The `cycles` HashSet is defined on line 863 and is NOT used to compute
    // the criterion `if visited.contains(start_node)` (line 870). A precise
    // backward slice must exclude it. (Line numbers re-synced to the current
    // deps.rs fixture — see sibling test.)
    if !Path::new(RUST_DEPS).exists() {
        eprintln!("skipping: {RUST_DEPS} not present");
        return;
    }
    let lines = slice_lines(RUST_DEPS, "detect_cycles", 870, "backward");
    assert!(
        !lines.contains(&863),
        "rust/IT3-rust-05: backward slice from 870 must exclude the unrelated `cycles` \
         definition on line 863; got {lines:?}"
    );
    // But the variables that DO feed the criterion must be present: `visited`
    // (def 866) and `start_node` (def 869).
    assert!(
        lines.contains(&866) && lines.contains(&869),
        "rust: backward slice from 870 must include the defs that feed it \
         (visited@866, start_node@869); got {lines:?}"
    );
}

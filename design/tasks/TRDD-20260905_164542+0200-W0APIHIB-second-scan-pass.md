...
- crates/tldr-mcp/src/cache.rs:426, crates/tldr-mcp/src/tools/mod.rs:1230, and the l2 daemon cache bench — three tests assert wall-clock latency targets (<10 µs per cache key, <15 µs per cache hit, and a concurrent-access bound) that a DEBUG build cannot meet reliably on a loaded machine. Verified pre-existing: the cache-hit test fails at the parent commit 7f50527 too (measured in a worktree of it), so the scan did not cause them. Decide per test: raise the debug-build target, gate them behind `#[cfg(not(debug_assertions))]`, or move them to a bench harness — the current form makes `cargo test` red for every contributor (found during the landing, missed by the first pass)

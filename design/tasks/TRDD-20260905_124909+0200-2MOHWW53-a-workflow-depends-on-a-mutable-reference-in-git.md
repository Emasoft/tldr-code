...
- 2026-09-05T21:33:13+0200 — APPROVED by the session Claude under the user's 2026-09-05 directive to decide from verified facts and implement what is good. Work is authorized; no push. Correction: cargo-dist exposes `[dist.github-action-commits]` (documented in its config reference) to pin the generated workflow's actions to full commit SHAs, so the finding is actionable through the generator's own config despite release.yml being autogenerated.
